document.addEventListener("DOMContentLoaded",()=>{const f=document.getElementById("slider-age"),m=document.getElementById("slider-tonnage"),L=document.getElementById("label-age"),I=document.getElementById("label-tonnage"),w=document.querySelectorAll("div[aria-label='Maintenance Selector'] .toggle-btn"),E=document.getElementById("metric-remaining"),v=document.getElementById("metric-eff-loss"),C=document.getElementById("metric-waste-cost"),k=document.getElementById("metric-capex-cost"),a=document.getElementById("lifecycle-status-badge"),A=document.getElementById("lifecycle-svg"),u=document.getElementById("planner-lead-form"),B=document.getElementById("plan_metrics_input");if(!f||!m||!E||!v||!C||!k||!a){console.warn("[PWA Client] Lifecycle Planner elements missing. Skipping initialization.");return}let g="yearly";f.addEventListener("input",()=>{L.textContent=`${f.value} Years`,y(),b()}),m.addEventListener("input",()=>{I.textContent=`${parseFloat(m.value).toFixed(1)} Tons`,y(),b()}),w.forEach(n=>{n.addEventListener("click",()=>{w.forEach(l=>l.classList.remove("active")),n.classList.add("active"),g=n.dataset.val,y(),window.ComfortAudio&&typeof window.ComfortAudio.playClick=="function"&&window.ComfortAudio.playClick()})});function b(){window.ComfortAudio&&typeof window.ComfortAudio.playTick=="function"&&window.ComfortAudio.playTick()}function y(){const n=parseInt(f.value),l=parseFloat(m.value);let s=.015,c=14;g==="none"?(s=.035,c=10):g==="twice"&&(s=.008,c=16);const r=Math.max(0,c-n),t=Math.min(100,Math.round(n*s*100)),$=Math.round(4e3+(l-1)*1400),h=l*300,o=Math.round(h*(t/100));E.textContent=r===0?"Expired (Replace Now)":`${r.toFixed(1)} Years`,v.textContent=`${t}% Loss`,C.textContent=`$${o} / year`,k.textContent=`$${$.toLocaleString()}`,r>10?(a.textContent="Excellent",a.style.background="#10B981"):r>5?(a.textContent="Good",a.style.background="#0B7A53"):r>2?(a.textContent="Caution",a.style.background="#F59E0B"):(a.textContent="Critical - Replace",a.style.background="#EF4444"),B.value=`[System Age: ${n} Years | Tonnage: ${l} Tons | Maintenance: ${g} | Remaining Life: ${r.toFixed(1)} Yrs | Eff Loss: ${t}%]`,M(n,c,s)}function M(n,l,s){const t={top:20,right:20,bottom:35,left:45},$=450-t.left-t.right,h=250-t.top-t.bottom,o=i=>t.left+i/15*$,e=i=>t.top+(1-(i-.5)/.5)*h;let d=[];for(let i=0;i<=15;i++){const z=Math.max(.5,1-i*s);d.push({x:o(i),y:e(z)})}let p=`M ${d[0].x} ${d[0].y}`;for(let i=1;i<d.length;i++)p+=` L ${d[i].x} ${d[i].y}`;const F=`${p} L ${o(15)} ${e(.5)} L ${o(0)} ${e(.5)} Z`,x=o(n),S=Math.max(.5,1-n*s),W=e(S);A.innerHTML=`
            <!-- Grid Lines -->
        ...
        `}u&&u.addEventListener("submit",n=>{const s=document.getElementById("fullname_plan").value.trim().split(" "),c=s[0]||"",r=s.slice(1).join(" ")||"Customer",t={fname:c,lname:r,tel:document.getElementById("phone_plan").value.trim(),email:document.getElementById("email_plan").value.trim(),city:document.getElementById("city_plan").value.trim(),message:`[AC Lifespan Planner Report] ${B.value} [Request] Client requests pre-failure replacement consultation.`,honeypot:document.getElementById("honeypot_plan").value};typeof window.submitFormWithSync=="function"?window.submitFormWithSync(n,u,t,()=>{alert("Consultation request secure! Chris or Sean will contact you to schedule an inspection."),u.reset(),typeof window.configurePushNotifications=="function"&&window.configurePushNotifications()}):console.error("submitFormWithSync not found in global scope")}),y()});
