document.addEventListener("DOMContentLoaded",()=>{const f=document.getElementById("slider-age"),u=document.getElementById("slider-tonnage"),A=document.getElementById("label-age"),M=document.getElementById("label-tonnage"),b=document.querySelectorAll("div[aria-label='Maintenance Selector'] .toggle-btn"),h=document.getElementById("metric-remaining"),p=document.getElementById("metric-eff-loss"),w=document.getElementById("metric-waste-cost"),E=document.getElementById("metric-capex-cost"),s=document.getElementById("lifecycle-status-badge"),B=document.getElementById("lifecycle-svg"),m=document.getElementById("planner-lead-form"),I=document.getElementById("plan_metrics_input");if(!f||!u||!h||!p||!w||!E||!s){console.warn("[PWA Client] Lifecycle Planner elements missing. Skipping initialization.");return}let g=!1,x="yearly";f.addEventListener("input",()=>{g=!0,A.textContent=`${f.value} Years`,y(),L()}),u.addEventListener("input",()=>{g=!0,M.textContent=`${parseFloat(u.value).toFixed(1)} Tons`,y(),L()}),b.forEach(n=>{n.addEventListener("click",()=>{g=!0,b.forEach(l=>{l.classList.remove("active"),l.setAttribute("aria-pressed","false")}),n.classList.add("active"),n.setAttribute("aria-pressed","true"),x=n.dataset.val,y(),window.ComfortAudio&&typeof window.ComfortAudio.playClick=="function"&&window.ComfortAudio.playClick()})});function L(){window.ComfortAudio&&typeof window.ComfortAudio.playTick=="function"&&window.ComfortAudio.playTick()}function y(){if(!g){h.textContent="Adjust inputs to calculate",p.textContent="\u2014",w.textContent="\u2014",E.textContent="\u2014",s.textContent="Awaiting Input",s.style.background="#64748b",B.innerHTML=`
            ...
            `;return}const n=parseInt(f.value),l=parseFloat(u.value);let a=.015,c=14;x==="none"?(a=.035,c=10):x==="twice"&&(a=.008,c=16);const r=Math.max(0,c-n),t=Math.min(100,Math.round(n*a*100)),v=Math.round(4e3+(l-1)*1400),C=l*300,o=Math.round(C*(t/100));h.textContent=r===0?"Expired (Replace Now)":`${r.toFixed(1)} Years`,p.textContent=`${t}% Loss`,w.textContent=`$${o} / year`,E.textContent=`$${v.toLocaleString()}`,r>10?(s.textContent="Excellent",s.style.background="#10B981"):r>5?(s.textContent="Good",s.style.background="#0B7A53"):r>2?(s.textContent="Caution",s.style.background="#F59E0B"):(s.textContent="Critical - Replace",s.style.background="#EF4444"),I.value=`[System Age: ${n} Years | Tonnage: ${l} Tons | Maintenance: ${x} | Remaining Life: ${r.toFixed(1)} Yrs | Eff Loss: ${t}%]`,F(n,c,a)}function F(n,l,a){const t={top:20,right:20,bottom:35,left:45},v=450-t.left-t.right,C=250-t.top-t.bottom,o=i=>t.left+i/15*v,e=i=>t.top+(1-(i-.5)/.5)*C;let d=[];for(let i=0;i<=15;i++){const W=Math.max(.5,1-i*a);d.push({x:o(i),y:e(W)})}let k=`M ${d[0].x} ${d[0].y}`;for(let i=1;i<d.length;i++)k+=` L ${d[i].x} ${d[i].y}`;const S=`${k} L ${o(15)} ${e(.5)} L ${o(0)} ${e(.5)} Z`,$=o(n),T=Math.max(.5,1-n*a),z=e(T);B.innerHTML=`
            <!-- Grid Lines -->
