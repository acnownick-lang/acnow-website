function initCalendarWidget(){const s=document.getElementById("dispatch-calendar-container");if(!s)return;function u(){const e=[],t=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],r=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];let a=new Date;for(a.setDate(a.getDate()+1);e.length<5;){const l=a.getDay();if(l!==0&&l!==6){const y=`${t[l]}, ${r[a.getMonth()]} ${a.getDate()}`;e.push(y)}a.setDate(a.getDate()+1)}return e}const m=u();let d="";const g=(e,t)=>[["Booked","Available","Booked"],["Available","Booked","Available"],["Booked","Available","Available"],["Available","Available","Booked"],["Booked","Booked","Available"]][e%5][t];let n=`
    ...
    `;m.forEach((e,t)=>{const r=e.split(", ");n+=`
            <div class="calendar-day-col" style="flex: 1 0 85px; text-align: center;">
                <div style="font-size: 12px; font-weight: 700; color: var(--primary); text-transform: uppercase; margin-bottom: 4px;">${r[0].substring(0,3)}</div>
                <div style="font-size: 13px; font-weight: 800; color: var(--dark); margin-bottom: 10px; border-bottom: 1px solid var(--gray-light); padding-bottom: 6px;">${r[1]}</div>
                
                <!-- Slots -->
                <div style="display: flex; flex-direction: column; gap: 8px;">
        `,["Morning (8am-12)","Afternoon (12-4)","Evening (4-8)"].forEach((l,p)=>{if(g(t,p)==="Booked")n+=`
    ...
    `,s.innerHTML=n;const c=s.querySelectorAll(".cal-slot-btn"),f=document.getElementById("calendar-feedback-box"),b=document.getElementById("cal-selected-txt"),i=s.closest("form");let o=null;i&&(o=i.querySelector("input[name='reserved_appointment_slot']"),o||(o=document.createElement("input"),o.type="hidden",o.name="reserved_appointment_slot",i.appendChild(o))),c.forEach(e=>{e.addEventListener("click",()=>{c.forEach(t=>{t.dataset.selected="false",t.style.background="var(--white)",t.style.color="#03543F",t.style.borderColor="#A7F3D0"}),e.dataset.selected="true",e.style.background="var(--primary)",e.style.color="var(--white)",e.style.borderColor="var(--primary)",d=e.dataset.slot,o&&(o.value=d),b.textContent=d,f.style.display="block",window.ComfortAudio&&typeof window.ComfortAudio.playClick=="function"&&window.ComfortAudio.playClick()})})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",initCalendarWidget):initCalendarWidget();
